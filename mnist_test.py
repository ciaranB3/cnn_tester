...
class BasicLitFirstBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BasicLitFirstBlock, self).__init__()
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.conv1 = conv3x3(inplanes, planes, stride)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
...
class BasicLitLastBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BasicLitLastBlock, self).__init__()
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.conv1 = conv3x3(inplanes, planes, stride)
        self.bn1 = nn.BatchNorm2d(planes)
        self.lit = LITnet(10.0, 4)
...
class BasicLitBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BasicLitBlock, self).__init__()
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.conv1 = conv3x3(inplanes, planes, stride)
        self.bn1 = nn.BatchNorm2d(planes)
        self.lit1 = LITnet(10.0, 4)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = nn.BatchNorm2d(planes)
        self.downsample = downsample
        self.stride = stride
        self.lit2 = LITnet(10.0, 4)
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, Bottleneck):
                    ...
                    nn.init.constant_(m.bn2.weight, 0)
                elif isinstance(m, BasicLitBlock):
                    nn.init.constant_(m.bn2.weight, 0)
                elif isinstance(m, BasicLitFirstBlock):
                    nn.init.constant_(m.bn2.weight, 0)
                elif isinstance(m, BasicLitLastBlock):
                    nn.init.constant_(m.bn2.weight, 0)
...
def litresnet18(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = LitResNet(BasicLitBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model

def litresnet20(pretrained=False, **kwargs):
